...
class MonitoringProvider extends ChangeNotifier {
  ...
  Future<void> onRefreshKolam() async {
    ...
    if (finishedTambakResponse is ApiResponseSuccess) {
      if (finishedTambakResponse.data.isNotEmpty) {
        ...
        _kolamResponse = _repository.getKolam(choosenTambak!.id);
      } else {
        _kolamResponse = Future.value(ApiResponseSuccess(data: <Kolam>[]));
      }
      notifyListeners();
    } else {
  ...
  void deleteKolam(String idKolam) async {
    if (deleteKolamResponse is! ApiResponseLoading) {
      ...
      deleteKolamResponse =
          await _repository.deleteKolam(idKolam: idKolam).then((value) {
        if (value is ApiResponseFailed) {
          Fluttertoast.showToast(
              msg: "Gagal menghapus kolam, periksa internet anda!");
        }
        return value;
      });
      onRefreshKolam();
